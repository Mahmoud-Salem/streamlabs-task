...
const OddNumStreams = () => {
    ...
    useEffect(() => {
        HTTP.get('oddViewersStreamSQL', {})
        .then(({data}) => {
            setGamesSQL(data.games);
            HTTP.get('oddViewersStreamCoding', {})
            .then(({data}) => {
    ...
    return (
            <div className="project-unapproved">
                <h1> Streamers with odd number of viewers.</h1>
